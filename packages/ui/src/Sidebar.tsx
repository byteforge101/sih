...
import {
  LayoutDashboard, Store, ShoppingCart, User, Users, BookOpen, HeartHandshake, Shield, GraduationCap, ChevronLeft, ChevronRight, LucideIcon, Video, Camera, HelpCircle,Vote, ClipboardEdit
} from 'lucide-react';
...
const studentLinks = [
  ...
  { href: '/mainapp/student-details', label: 'Predict Score', icon: ClipboardEdit },
...
export default function Sidebar({ session }: { session: Session | null }) {
  // 1. Default to a consistent state (e.g., expanded) for both server and client
  const [isCollapsed, setIsCollapsed] = React.useState(false);
  const pathname = usePathname();
  let navLinks: { href: string; label: string; icon: LucideIcon; }[] = [];

  // 2. Use useEffect to set the initial state based on window size, ONLY on the client
  React.useEffect(() => {
    const checkWindowSize = () => {
      if (window.innerWidth < 1280) { // xl breakpoint, you can adjust this
        setIsCollapsed(true);
      } else {
        setIsCollapsed(false);
      }
    };
    checkWindowSize(); // Check on initial mount
    window.addEventListener('resize', checkWindowSize); // Check when window is resized
  ...
  return (
    <motion.aside
      ...
      initial={false} // 3. Set initial to false to prevent animation mismatch on first render
      animate={isCollapsed ? "collapsed" : "expanded"}
      className="relative flex flex-col h-full bg-white/70 backdrop-blur-xl border-r border-slate-200/50 shadow-2xl shadow-blue-500/5"
    >
      <div className="p-4 flex items-center justify-between border-b border-slate-200/50">
          <AnimatePresence>
            ...
            <motion.div initial={{opacity: 0}} animate={{opacity: 1, transition: {delay: 0.2}}} exit={{opacity: 0}} className="flex items-center gap-3 overflow-hidden">
                <div className="w-10 h-10 bg-gradient-to-r from-blue-700 to-blue-500 rounded-full flex items-center justify-center shadow-inner-lg">
                    <GraduationCap size={24} className="text-white"/>
                </div>
                <span className="text-2xl font-bold bg-gradient-to-r from-blue-500 to-blue-700 bg-clip-text text-transparent whitespace-nowrap">Learnova</span>
            </motion.div>
            )}
          </AnimatePresence>
          <motion.button whileHover={{scale: 1.1}} whileTap={{scale: 0.9}} onClick={() => setIsCollapsed(!isCollapsed)} className="p-2 rounded-full bg-white/50 hover:bg-blue-100/50 transition-colors shadow-sm border border-slate-200/50">
            ...
            <motion.div key={link.label} custom={i} variants={navItemVariants} initial="hidden" animate="visible">
                <Link href={link.href} title={link.label} className="relative block">
                    <motion.div whileHover={{ scale: 1.02 }} className={`flex items-center p-3 rounded-xl cursor-pointer transition-all duration-300 group ${
                        isActive ? 'bg-gradient-to-r from-blue-700 to-blue-500 text-white shadow-lg shadow-blue-500/30' : 'text-slate-500 hover:bg-white hover:shadow-md'
                      }`}>
                        <Icon size={isCollapsed ? 24 : 20} className={`transition-all duration-300 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-blue-500'}`} />
                        <AnimatePresence>
                          {!isCollapsed && (
